<x-app-layout>
    ...
    <section class="hidden pb-4 fixed bottom-0 mr-4 right-0  z-50 transition duration-500 delay-3000 ease-in-out opacity-0"
    id="classAd"
    x-data="{$open : true}"
    x-show="$open"
    x-cloak>
        <div class=" w-11/12 md:w-full max-w-2xl mx-auto py-8 px-6 md:px-12  bg-yellow-400 rounded-2xl md:flex shadow-2xl bg-hero-pattern bg-cover bg-center">
            <div class="md:mr-8 flex-1">
                <h3 class="text-yellow-100 bg-red-700 uppercase font-bold mb-0 inline-block text-base px-4 py-1 rounded-xl">MasterClass 100% gratis</h3>
                <h2 class=" text-accent-400 text-3xl my-4 md:my-2 md:text-6xl font-extrabold leading-none">
                    LA <span class="text-red-700">MEJOR</span> DIETA
                </h2>
                <p class="text-gray-900 text-sm md:text-base mb-4">Este 1, 2 y 3 de junio descubrirás entre las diferentes dietas que existen en el mundo cuál es la mejor dieta Para bajar de peso que te convertirá en una verdadera maquina quema grasa</p>
                <div class=" text-center md:flex items-center">
                    <a href="https://doctorbayter.ac-page.com/dkp" target="_blank" class="inline-block font-bold px-8 py-2 text-lg border border-red-700  bg-red-700 bg-accent-400 text-yellow-400 rounded-xl hover:bg-transparent hover:text-red-700" >Apuntate aquí</a>
                    <button x-on:click="$open = !$open; sessionStorage.setItem('classAd', false)" class=" mt-4 md:ml-3 md:mt-0 inline-block font-bold text-lg text-red-700 hover:underline outline-none" >No me interesa</button>
                </div>
            </div>
