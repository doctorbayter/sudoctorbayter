...
class UserPlan extends Component
{
    ...
    public function mount(){
        ...
        if (auth()->user()->id == 3420 || auth()->user()->id == 1) {
            $this->user_retos = auth()->user()->fases->whereNotIn('id', [1, 2, 3, 4, 5, 7, 11, 14, 15, 16])->sortBy('id');
        } else {
            $this->user_retos = auth()->user()->fases->whereNotIn('id', [1, 2, 3, 4, 5, 7, 9, 10, 11, 13, 14, 15, 16])->sortBy('id');
        }
